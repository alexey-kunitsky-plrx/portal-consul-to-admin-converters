...
### Служебный флаг `disabled`

Соглашение по проекту: если сущность есть в исходнике, но её нужно скрыть (например, ссылается на устаревший / удалённый сервис), вместо удаления записи или пустой связи **ставится `featureFlag: "disabled"`**. Сам флаг `disabled` должен быть заведён в Strapi один раз руками с `availability: "none"` (`branches: []`) — после этого все импортеры цепляют его как обычную релейшн.

Зачем так: запись в данных остаётся (легко вернуть, видно историю), а пользователь её не увидит. Применяется в `admin-customer-service-converter` (например, услуги, которые раньше шли через freshservice).
...
**Фича-флаги:** `showFeat` на уровне item или кнопки → имя-код. `publish.js` резолвит строку в id реляции; отсутствующие флаги **не создаются автоматически** — печатается список missing-кодов, заводить руками в админке и перезапускать. Для скрытых услуг (например, бывшие freshservice-обёртки) используется служебный `disabled` — см. раздел «Служебный флаг `disabled`» выше.

**Режим обновления (`UPDATE_EXISTING`).** По умолчанию `publish.js` пропускает сущности, которые уже есть в Strapi (поиск по `section_code` / `serviceId`). Если нужно догнать изменения в данных (например, переименовали `featureFlag`), запускать с `UPDATE_EXISTING=true node publish.js` — тогда вместо `skip` идёт `PUT /{documentId}?locale=ru` + `PUT ...?locale=en` для обеих локалей. В сводке появятся отдельные блоки `Updated categories` / `Updated services`.
