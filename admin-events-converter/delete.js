...
async function deleteCollection(endpoint, label, labelField = "name") {
...
async function deleteAll() {
  ...
  const events = await deleteCollection("events", "events");
  ...
  log.success(`Events deleted: ${events.deleted}`);
  if (events.errors > 0) log.warning(`  errors: ${events.errors}`);
